...
if ($type === 'music') {
    // music may be a separate table in your schema
    // if you don't have a music table, adjust this to query media WHERE type='music'
    try {
        $stmt = $pdo->prepare("SELECT * FROM music ORDER BY title");
        $stmt->execute();
        $media = $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (Exception $e) {
        // fallback to media table if music doesn't exist
...
<!-- ===== MiniMedia Server Control Section ===== -->
        <form method="POST" class="d-flex align-items-center justify-content-between flex-wrap gap-2">
  <div class="btn-group" role="group">
    <button type="submit" name="action" value="start" class="btn btn-success">Start Server</button> 
    <button type="submit" name="action" value="stop" class="btn btn-danger">Stop Server</button>
  </div>
  <div class="ms-auto">
...
<?php else: ?>
  ...
  <div class="media-grid">
    ...
    if ($type === 'tv'):
      ...
      if (!empty($media)):
        foreach ($media as $m):
          // For music table rows may not have 'path' etc.
