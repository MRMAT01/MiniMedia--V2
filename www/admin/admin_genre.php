...
// Fetch all genres
$genres = $pdo->query("SELECT * FROM genres ORDER BY name")->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>MiniMedia - Genre</title>
...
.card, .table{background:#fff; color:#000; padding:15px; border-radius:6px;}
.sidebar{background:rgba(0,0,0,0.6);padding:10px;}
.sidebar .btn{width:100%; text-align:left; margin-bottom:4px;}
...
<div class="card mb-4 p-3">
    <form method="post" class="d-flex gap-2">
        <input type="text" name="name" class="form-control" placeholder="New genre name" required>
        <button type="submit" name="add_genre" class="btn btn-success">Add</button>
    </form>
</div>
...
<tr>
    ...
    <td>
        <form method="post" class="d-flex gap-2">
            ...
            <input type="text" name="name" value="<?= htmlspecialchars($g['name']) ?>" class="form-control" required>
            <button type="submit" name="edit_genre" class="btn btn-primary btn-sm">Save</button>
        </form>
    </td>
    <td>
        <a href="?delete=<?= $g['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Delete this genre?')">Delete</a>
    </td>
</tr>
<?php endforeach; ?>
</tbody>
