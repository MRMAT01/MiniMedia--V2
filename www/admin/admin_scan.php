...
<head>
<meta charset="utf-8">
<title>MiniMedia - Bulk Scan</title>
...
<div class="container">
  ...
  <p>This tool scans your <strong>/import</strong> for Movies, TV, Featured, and Music folders. Upload below or FTP files to folders first. It converts media, downloads covers/backdrops, and inserts to database, metadata to folder automatically. Large collections may require extended PHP execution time.</p>

  <!-- Upload Section -->
  <div class="card-section">
    <h3>Upload Media to Import Folder</h3>
    <div class="mb-3">
      <label for="uploadFolder" class="form-label">Select target folder:</label>
    ...
    <button id="uploadBtn" class="btn btn-success mb-2">Upload Files</button>
    <div class="progress mb-2" style="display:none;">
      <div class="progress-bar" id="uploadProgress">0%</div>
    </div>
    <div class="table-responsive">
      <table class="table table-striped table-dark" id="uploadResults" style="display:none;">
        <thead>
          <tr>
            <th>File</th>
            <th>Status</th>
  ...
  <div class="card-section">
    <h3>Scan Library</h3>
    <label for="scanType">Select type to scan:</label>
    <select id="scanType" class="form-select mb-2">
      <option value="movies">Movies</option>
      <option value="tv">TV Shows</option>
      <option value="featured">Featured</option>
      <option value="music">Music</option>
    </select>
    <button id="startScan" class="btn btn-primary mb-2">Start Scan</button>
    <div class="progress mb-2" style="display:none;">
      <div class="progress-bar" id="progressBar">0%</div>
    </div>
    <div class="table-responsive">
      <table class="table table-striped table-dark" id="resultsTable" style="display:none;">
        <thead>
          <tr>
            <th>Title</th>
            <th>Type</th>
...
document.getElementById('uploadBtn').addEventListener('click', async () => {
    ...
    const tbody = document.querySelector('#uploadResults tbody');
    tbody.innerHTML = '';
    document.getElementById('uploadResults').style.display = 'table';
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        formData.append('folder', folder); // new folder field

        try {
            const res = await fetch('admin_upload_to_import.php', {
                method: 'POST',
                body: formData
            });
            ...
            const row = document.createElement('tr');
            row.innerHTML = `<td>${file.name}</td><td>Failed</td><td>${err.message}</td>`;
            tbody.appendChild(row);
        }
    }
...
document.getElementById('startScan').addEventListener('click', async function(){
  ...
  const type = document.getElementById('scanType').value; // get selected type

  try {
    const res = await fetch(`dir_scan.php?type=${document.getElementById('scanType').value}`); // send type to PHP
    const data = await res.json();

    if(!data.results || !Array.isArray(data.results)){
      tbody.innerHTML = `<tr><td colspan="4">Error: ${data.error || 'Unexpected response'}</td></tr>`;
      progressBar.style.width = '100%';
    ...
    data.results.forEach(item => {
      const row = document.createElement('tr');
      row.innerHTML = `
        <td>${item.title || item.source || 'N/A'}</td>
        <td>${item.type || 'unknown'}</td>
        <td class="status-${item.status}">${item.status}</td>
        <td>${item.error ? item.error : (item.db_path || '')}</td>
      `;
      tbody.appendChild(row);
